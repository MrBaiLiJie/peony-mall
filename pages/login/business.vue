	...
	<view>
		...
		<view class="form">
			<view class="username">
				...
				<input placeholder="请输入手机号" v-model="userPhone" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<!-- <view class="code">
			...
			<view class="password">
				<input placeholder="请确认密码" v-model="passWord2" password=true placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="bussinfo">企业信息</view>
			<view class="busname">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 	uni.request({
			// 		method:'get',
			// 		url:`http://192.144.170.210:8085/sso/getAuthCode?telephone=${this.userPhone}`,
			...
			doReg(){
				...
				setTimeout(()=>{
					uni.getStorage({
						...
						success:(res)=>{
							//增加记录，密码md5
							res.data.push({username:this.userPhone,passwd:md5(this.passWord)})
							uni.setStorage({
								key: 'UserList',
								data: res.data,
								success: function () {
						...
						fail:(e)=>{
							...
							uni.setStorage({
								key: 'UserList',
								data: [{username:this.userPhone,passwd:md5(this.passWord)}],
								success: function () {
									uni.hideLoading()
									uni.showToast({title: '注册成功',icon:"success"});
									setTimeout(function(){
