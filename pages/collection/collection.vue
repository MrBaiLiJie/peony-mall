<template>
	<view>
		...
		<view class="goods-list">
			<view class="tis" v-if="goodsList.length==0&&!loading">收藏是空的哦~</view>
			<view class="row" v-for="(row,index) in goodsList" :key="index"  @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)"
				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']">
					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="goods-info">
							...
							<!-- <view class="spec">{{row.productSubTitle}}</view> -->
							<view class="price-number">
								<view class="price">￥{{row.pmsProduct.price}}</view>
								<!-- <view class="number">
...
<script>
	export default {
		...
		methods: {
			...
			// 	var ductId = this.goodsList[index];
			// 	uni.setStorage({
			// 		key:'ductId',
			// 		data:ductId,
			// 		success: () => {
			...
			deleteGoods(row) {
				...
				_this.$ajax.postJSON({
					...
					data:{
						userId:_this.userInfo.id,
						productId:_this.goodsList.productId
					}
				}).then(res=>{
