<template>
	<view>
		<view class="headerlist" :style="{position:headerPosition,top:headerTop}">
			...
			<view class="product-list">
				<view class="product" v-for="(goods) in goodsList" :key="goods.id" @tap="toGoods(goods.id)">
					...
					<view class="info">
						...
						<view class="slogan">库存{{goods.stock}}件</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			searchData() {
				...
				let _this = this;
				_this.$ajax.get({
					url: `/goods/select/ByName`,
					param: {
						name: this.key,
						pageSize: 10,
						pageNum: _this.pageNum,
						orderByType:this.paixu
					}
				}).then(res => {
					uni.hideLoading()
					if (_this.pageNum > 1) {
						_this.goodsList.concat(res.data.data);
					} else {
			...
			getGoodsList() {
				...
				let _this = this;
				_this.$ajax.get({
					url: `/goods/selectGoodsCategoryIdOrderBysort`,
					param: {
						categoryId: _this.classflyId,
						pageSize: 20,
						pageNum: _this.pageNum,
						orderByType:this.paixu
					}
