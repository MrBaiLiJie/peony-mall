<template>
    <view>
        ...
        <view class="goods-list">
            <view class="tis" v-if="!goodsList.length&&loading">
            ...
            <view class="row" v-for="(row,index) in goodsList" :key="index" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
                ...
                <view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']">
                    ...
                    <view class="goods-info" @tap="toGoods(row)">
                        <view class="img">
                            <image :src="row.pmsProduct.pic" mode="aspectFit"></image>
                        </view>
                        <view class="goods-info">
                            <view class="title">{{row.pmsProduct.name}}</view>
                            <view class="spec">{{row.sku ? row.sku.value : "未选择"}}</view>
    ...
    export default {
        ...
        methods: {
			...
			getCartData(){
				...
				this.showLoading()
				this.$ajax.get({
					url:`/cart/list?memId=${this.userInfo.id}`
				}).then(res=>{
			...
			deleteGoods(id){
				this.$ajax.post({
					url:`/cart/delete?ids=${id.join(',')}&memberId=${this.userInfo.id}`
				}).then(res=>{
					uni.showToast({
						title:"删除成功"
					})
            ...
            sum(){
                ...
                for(let i=0;i<len;i++){
                    if(this.goodsList[i].selected) {
                        this.sumPrice = this.sumPrice + (this.goodsList[i].quantity*this.goodsList[i].sku. price);
                    }
                }
